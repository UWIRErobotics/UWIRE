...
#define VW_RX_RAMP_LEN 160					// The size of the receiver ramp. Ramp wraps modulu this number
#define VW_RX_SAMPLES_PER_BIT 8				// Number of samples per bit
...
extern "C"
{
    // Set the digital IO pin to be for transmit data (default 12)
    extern void vw_set_tx_pin(uint8_t pin);

    // Set the digital IO pin to be for receive data  (default 11)
    extern void vw_set_rx_pin(uint8_t pin);

    // Set the digital IO pin to enable transmitter (press to talk) (default 10)
