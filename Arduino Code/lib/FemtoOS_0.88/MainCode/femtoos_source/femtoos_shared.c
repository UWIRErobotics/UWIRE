...
#if (defPriorityConstant == cfgTrue) && (defInterruptStartConstant == cfgTrue) && (defTaskIncludeConstant == cfgTrue)
  #define STATUSPARAMS(NR)
#else
  #define STATUSPARAMS(NR) ( ( (TaskInclude(TN_ ## NR)) & defStopStartMask) | ((InterruptStart(TN_ ## NR)) & (defInterruptStartMask)) | ( (Priority(TN_ ## NR)) << defBasePrioShift) ),
#endif
...
#define TDB_DEF(NR) { REGPARAMS(NR) STATUSPARAMS(NR) STACKSIZEPARAMS(NR) STACKOFFSETPARAMS(NR) TIMESLICEPARAMS(NR) SLOTDEPTHPARAMS(NR) TASKNAMEPARAMS(NR) };
 ...
 * is the first objectfile linked. Now, if the sum  of all stacks does not exceed 0xFF
 * we safely ignore the stackpointer __SP_H__ for calculations.
...
#define TASK_DEF(NR)                                                                                         \
  ...
  const Tchar Stack ## NR[StackSize(TN_  ## NR)] __attribute__ ((nocommon));                                 \
  extern void AppInit(TN_ ## NR) (void);                                                                     \
  extern void AppLoop(TN_ ## NR) (void);                                                                     \
  extern void AppBark(TN_ ## NR) (void);                                                                     \
  const TtaskDefinitionBlock PROGMEM tdb ## NR =  TDB_DEF(NR)
