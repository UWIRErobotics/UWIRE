...
void portShowError(Tuint08 uiMessage, Tuint08 uiInfoTask)
{ /* We display the error code on devErrorComPORT. If you use the stk500 with eight leds, this can be
   * seen quite nicely. Specify that we want the message to be repeated 16 times. We arive here
   * with disabled tick and global interrupts. */
  Tuint08 uiCount = 16;
  /* Port A must be configured as output on all pins. We do not bother to store its original
   ...
   * blinking scheme. */
  while (uiCount--)
  { /* First display the message (6 bits) with the leftmost led on */
    devErrorComPORT = ~ (errSignLeft | uiMessage);
    /* wait a little while */
    portShortDelay(0x08);
    /* Then display the message (6 bits) with the led right to the leftmost on, this gives a
     * blinking effect which cannot be missed. */
    devErrorComPORT = ~ (errSignRight | uiMessage);
    /* wait a little while */
    portShortDelay(0x08);
    /* Now show in which info and task the error occurred. */
...
#if ((cfgUseEquidistantTicks == cfgTrue) && ((cfgCheckTiming == cfgTrue) || (cfgUseLoadMonitor == cfgTrue))) || ((cfgUseLoadMonitor == cfgTrue) && (defUseIsrStack == cfgTrue))
...
{ /* Check if the timer interrupt is due to fire. If so return true and release the trigger, otherwise
   ...
   * and utilize the none zero result as true value. */
 ...
 * stack pointer, global interrupts must be switched off.
...
{ asm volatile (
   ...
   "sts  %[_ISR3_], r31      \n\t" /* Place it on the new isr stack in the correct position.                                         */
  #endif
   "lds  r31,%[_Status_]     \n\t" /* Load the uiOSstatus, now indicating we are running a task.                                     */
...
#if (cfgIntUserDefined == cfgTrue) && (includeIsrBegin == cfgTrue)
...
#if (cfgIntUserDefined == cfgTrue) && (includeIsrEndReturn == cfgTrue)
...
#if (cfgIntUserDefined == cfgTrue) && (cfgIntOsProtected == cfgTrue) && (includeIsrEndYield == cfgTrue) && (includeTaskYield == cfgTrue)
...
{ /* PortSaveContext must do what it promises, thus save all used registers for  the current task
  ...
  asm volatile (
   ...
   "ldi  r31,0x00            \n\t" /* Clear r31 to compare with and to collect the register                                          */
   "cpse r30,r31             \n\t" /* test if we may continue, only if the value of uiStackTCheck == 0, if not zero (r31!=r30), we   */
   "rjmp 8f                  \n\t" /* do not save the stack and leave asap, the task will be terminated, registers are not checked   */
