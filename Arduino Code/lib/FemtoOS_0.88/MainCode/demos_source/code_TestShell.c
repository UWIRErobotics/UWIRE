...
{ Tuint08 uiCommand = getSingleValue();
  ...
  else if (uiCommand == defRequestKill)         HandleCommand(taskKillTask);
  else if (uiCommand == defRequestRestart)      HandleRestart();
  ...
  /* else if (uiCommand == defRequestTest)         SendSingleResponse(defResponseOK); */
...
{ taskQueuWriteRequestOnName(WriteQueuT4,0);
  genQueuClearOnName(WriteQueuT4);
  taskQueuReleaseOnName(WriteQueuT4);
  taskQueuWriteRequestOnName(ReadQueuT4,0);
...
#define ledOff(lednr)  { PORTA |= (1 << lednr); }
#define ledOn(lednr)   { PORTA &=  ~(1 << lednr); }
/* Note: function below is not thread save only call when interrupts are off */
#define ledInv(lednr)  { PORTA ^= (1 << lednr); }

/*
static void ledOff(Tuint08 lednr)
{ taskEnterGlobalCritical();
  PORTA |= (1 << lednr);
  taskDelayFromNow(100);
  taskExitGlobalCritical(); }

static void ledOn(Tuint08 lednr)
{ taskEnterGlobalCritical();
  PORTA &=  ~(1 << lednr);
  taskDelayFromNow(100);
  taskExitGlobalCritical(); }

static void ledInv(Tuint08 lednr)
{ taskEnterGlobalCritical();
  PORTA ^= (1 << lednr);
  taskDelayFromNow(100);
  taskExitGlobalCritical(); }
...
{ Tuint08 uiPerOff = uiPeriodOff[uiLed];
  ...
  if (uiState == defDemoRequestLedStateOff)
  { ledOff(uiLed);
    taskDelayFromNow(100); }
  if (uiState == defDemoRequestLedStateOn)
  { ledOn(uiLed);
    /* activate this if you want to simulate an error on led state on */
    /* asm("clr r3"::); */
    /* asm("clr r7"::); */
    taskDelayFromNow(100); }
  if (uiState == defDemoRequestLedStateFlash)
  { ledOff(uiLed);
    taskDelayFromNow((((Tuint16) uiPerOff)<<5));
    ledOn(uiLed);
    taskDelayFromNow((((Tuint16 )uiPerOn)<<5)); }
  if (uiState == defDemoRequestLedStateFreeze)
  { while(--uiPerOn); }
