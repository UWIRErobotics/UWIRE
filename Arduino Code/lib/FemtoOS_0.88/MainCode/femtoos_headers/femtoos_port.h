 ...
 * true for other assembler parts of the OS. The memory clober ensures uncaching of variables
...
#define portSetReducedStack(field) \
  asm volatile ( "out __SP_L__, %A0 " : : "r" (field) )
 ...
 * which contains the bits that must be set in the prescale register.
 */
