 ...
 * tasks (a shared boolean for instance)
 */
 ...
 *
 * If you need protection from context switching during part of your task, you need
 * to activate this option. Then you can switch off tick interrupts and still task
 ...
 * of the stack or real register usage. Note that if you activate cfgCheckRegisters
 ...
 * is de-activated the use of all registers is monitored. Furthermore, activating
 ...
 * If you make use of timeouts on synchronization primitives, you must also
 * enable this option.
 *
 ...
 * only (!) running tasks are watched. A blocking, delayed of sleeping task can never
 ...
 * dynamically loaded or unloaded, so it is know on any time which tasks
 ...
 * to cfgTrue. You code is run with tick interrupts disabled, so be very brief.
...
/** <!--0608-->
 * Enable initialization hook called before context construction of each task.
 *
 ...
 * possible to set both callMainInit and callAppInit to cfgTrue. The appBoot()
 ...
 */
#define  includeTaskSuspendMe                    cfgTrue
 ...
 */
#define  includeTaskKillTask                     cfgTrue
...
/** <!--0723-->
 * Send a byte or marker trough the trace.
 *
 * Activate cfgCheckTarce to make use of the this facility.
 * includes genTraceInfo() and genTraceMarker().
 */
 ...
 * switching of tasks, and keep track of the interrupt state over a context switch.
 */
...
/** <!--0822-->
 * Specify which tasks must be included and how they are started.
 *
 * You can specify one default TaskInclude value for all tasks by using
 * TaskIncludeDefault. If you want to specify the TaskInclude setting per task,
 * set TaskIncludeDefault to cfgDefaultNon, and define the value for each
 * task separately. If you define each task as well as the TaskIncludeDefault
 * the latter takes precedence, so the default value overrides values specified
 * per task (if unequal to cfgDefaultNon). Defining this value is not required.
 *
 * See TaskInclude_[taskname] for explanation.
 */
#define  TaskIncludeDefault                      cfgDefaultNon
 ...
 * other possibilities are #define cfgStartTerminated, cfgStartSleeping,
 * cfgStartSuspended and cfgStartRunning which behave as can be expected
 * by there definition. Thus, normally you would use which implies the
 * task is started directly at startup.
 *
...
/** <!--0830-->
 * Specify the capabilities of each task.
 *
 * You can specify one default Capabilities value for all tasks by using
 * CapabilitiesDefault. If you want to specify the Capabilities setting per task,
 * set CapabilitiesDefault to cfgDefaultNon, and define the value for each
 * task separately. If you define each task as well as the CapabilitiesDefault
 * the latter takes precedence, so the default value overrides values specified
 * per task (if unequal to cfgDefaultNon). Defining this value is not required.
 *
 * See Capabilities_[taskname] for explanation.
 */
#define  CapabilitiesDefault                     cfgDefaultNon
 ...
 * checking. Thus, if your task should not require no more as 14 bytes,
 ...
 * The value is limited to the value that, increased with the highest
 * stack size, does not exceed the 255 or 65535 limit (one or two byte
 ...
 *
 * The Femto OS is not really stack hungry and the stack is also used to hold a
 * number of background variables when the tasks are running. The code for these
 * background variables never uses more than 8 bytes with all protection switched
 * off, and 16 bytes with protection switched on. However, please experiment with
 * the different values and have cfgCheckOsStack == cfgTrue while testing. This
 * is because the stack itself may need to be larger. The StackSizeOS may not
 * be larger then 255.
 */
 ...
 * Value must be between 0 and 65536 and is in fact limited by the amount
 * or ram the device has.
 */
#define  StackSizeISR                            8

/** <!--0823-->
 * Specify which stack sizes must be used for each task.
 *
 * You can specify one default StackSize value for all tasks by using
 * StackSizeDefault. If you want to specify the StackSize setting per task,
 * set StackSizeDefault to cfgDefaultNon, and define the value for each
 * task separately. If you define each task as well as the StackSizeDefault
 * the latter takes precedence, so the default value overrides values specified
 * per task (if unequal to cfgDefaultNon). Defining this value is not required.
 *
 * This default is not used to define StackSizeISR or StackSizeOS.
 * See StackSize_[taskname] for explanation.
 */
#define  StackSizeDefault                        cfgDefaultNon
 ...
 *    save nothing on the stack. Two af these bytes are removed again, the
 ...
 * -- If you use any calls to the OS, they usually disable interrupts after
 *    the (nested) call. If the function takes parameters, they are usually
 *    pushed on the stack in between. Count one byte per parameter.
 ...
 * as 255 bytes however. In that case two bytes are needed per task to store
 * the information.
 */
...
/** <!--0824-->
 * Specify which time slices must be used for each task.
 *
 * You can specify one default TimeSlice value for all tasks by using
 * TimeSliceDefault. If you want to specify the TimeSlice setting per task,
 * set TimeSliceDefault to cfgDefaultNon, and define the value for each
 * task separately. If you define each task as well as the TimeSliceDefault
 * the latter takes precedence, so the default value overrides values specified
 * per task (if unequal to cfgDefaultNon). Defining this value is not required.
 *
 * This default is not used to define TimeSliceIdleTime.
 * In that case the value specified here is ignored.
 * See TimeSlice_[taskname] for explanation.
 */
#define  TimeSliceDefault                        cfgDefaultNon
 ...
 *
 * There is no possibility to define a default task name.
 */
...
/** <!--0826-->
 * Specify which priority must be set when each task per default.
 *
 * You can specify one default priority value for all tasks by using
 * PriorityDefault. If you want to specify the priority setting per task,
 * set PriorityDefault to cfgDefaultNon, and define the value for each
 * task separately. If you define each task as well as the PriorityDefault
 * the latter takes precedence, so the default value overrides values specified
 * per task (if unequal to cfgDefaultNon). Defining this value is not required.
 *
 * See Priority_[taskname] for further explanation.
 */
#define  PriorityDefault                         cfgDefaultNon
...
/** <!--0827-->
 * Specify which registers are used by each task per default.
 *
 * You can specify one default RegisterUse value for all tasks by using
 * RegisterUseDefault. If you want to specify the RegisterUse setting per task,
 * set RegisterUseDefault to cfgDefaultNon, and define the value for each
 * task separately. If you define each task as well as the RegisterUseDefault
 * the latter takes precedence, so the default value overrides values specified
 * per task (if unequal to cfgDefaultNon). Defining this value is not required.
 *
 * See RegisterUse_[taskname] for further explanation.
 */
#define  RegisterUseDefault                      cfgDefaultNon
...
/** <!--0828-->
 * Specify which registers must be checked per default for each task.
 *
 * You can specify one default RegisterCheck value for all tasks by using
 * RegisterCheckDefault. If you want to specify the RegisterCheck setting per task,
 * set RegisterCheckDefault to cfgDefaultNon, and define the value for each
 * task separately. If you define each task as well as the RegisterCheckDefault
 * the latter takes precedence, so the default value overrides values specified
 * per task (if unequal to cfgDefaultNon). Defining this value is not required.
 *
 * See RegisterCheck_[taskname] for further explanation.
 */
#define  RegisterCheckDefault                    cfgDefaultNon
...
/** <!--0829-->
 * Specify which interrupts must be activated per default.
 *
 * You can specify one default InterruptStart value for all tasks by using
 * InterruptStartDefault. If you want to specify the InterruptStart setting per task,
 * set InterruptStartDefault to cfgDefaultNon, and define the value for each
 * task separately. If you define each task as well as the InterruptStartDefault
 * the latter takes precedence, so the default value overrides values specified
 * per task (if unequal to cfgDefaultNon). Defining this value is not required.
 *
 * See InterruptStart_[taskname] for further explanation.
 */
#define  InterruptStartDefault                   cfgDefaultNon
 ...
 * when InterruptStartDefault is set to cfgDefaultNon, any other value takes
...
/** <!--0922-->
 * Specify which slot sizes must be used for each task per default.
 *
 * You can specify one default SlotSize value for all tasks by using
 * SlotSizeDefault. If you want to specify the SlotSize setting per task,
 * set SlotSizeDefault to cfgDefaultNon, and define the value for each
 * task separately. If you define each task as well as the SlotSizeDefault
 * the latter takes precedence, so the default value overrides values specified
 * per task (if unequal to cfgDefaultNon). Defining this value is not required.
 *
 * See SlotSize_[taskname] for explanation.
 */
#define  SlotSizeDefault                         cfgDefaultNon
...
/** <!--0923-->
 * Specify the default use of each slot.
 *
 * You can specify one default SlotUse value for all tasks by using
 * SlotUseDefault. If you want to specify the SlotUse setting per task,
 * set SlotUseDefault to cfgDefaultNon, and define the value for each
 * task separately. If you define each task as well as the SlotUseDefault
 * the latter takes precedence, so the default value overrides values specified
 * per task (if unequal to cfgDefaultNon). Defining this value is not required.
 *
 * See SlotUse_[slotname] for explanation.
 */
#define  SlotUseDefault                          cfgDefaultNon
...
/** <!--0924-->
 * Specify the size of for each queue per default.
 *
 * You can specify one default QueuSize value for all tasks by using
 * QueuSizeDefault. If you want to specify the QueuSize setting per task,
 * set QueuSizeDefault to cfgDefaultNon, and define the value for each
 * task separately. If you define each task as well as the QueuSizeDefault
 * the latter takes precedence, so the default value overrides values specified
 * per task (if unequal to cfgDefaultNon). Defining this value is not required.
 *
 * See QueuSize_[slotname] for explanation.
 */
#define  QueuSizeDefault                         cfgDefaultNon
...
/** <!--0832-->
 * Specify which file space must be reserved for each file per default.
 *
 * You can specify one default FileSpace value for all tasks by using
 * FileSpaceDefault. If you want to specify the FileSpace setting per task,
 * set FileSpaceDefault to cfgDefaultNon, and define the value for each
 * task separately. If you define each task as well as the FileSpaceDefault
 * the latter takes precedence, so the default value overrides values specified
 * per task (if unequal to cfgDefaultNon). Defining this value is not required.
 *
 * This default is not used to define FileSpaceStandard.
 * See FileSpace_[filename] for explanation.
 */
#define  FileSpaceDefault                        cfgDefaultNon
