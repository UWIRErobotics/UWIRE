 ...
 * that task is changed to suspended immediately. This call does not force
 * a context switch, thus if you call it with the current running task number,
 * the method still returns and will not be scheduled again only after a
 * context switch.
 * The call has no effect on terminated, sleeping or tasks that already
 * suspended. It will not work on blocked tasks either, since at a resume
 * there is no guarantee that the task will be released when possible.
 * You may call suspend on running tasks that hold slots, but the tasks
 * that are waiting their release will not be released until this task is
 * resumed, which may be indefinitely.
 */
 ...
 *
 * If you need to suspend your task immediately call this method. The
 ...
 * on your task. If you want a task in suspended mode, do not call this
 * from an init routine, but from the beginning of the infinity loop.
 */
void taskSuspendMe(void) defSysWrapperEpilogue;
 ...
 * woken by the user anymore (it can be restarted though). If all
 ...
 * is brought to the same state. A terminated task can be restarted, provided
 * you compiled the necessary code in. See taskResart for more information.
 */
void taskKillTask(Tuint08 uiTaskNumber) defSysWrapperEpilogue;
#define taskKillTaskOnName(TaskName) taskKillTask(preTaskNumberOf(TaskName))

/** <!--0121-->
 * Restart one particular task.
 *
 * Restart a task by recreating its context. Init or bark are NOT called
 * again, these are for one time (hardware) initialization. The context is
 * cleared, locks are released, and the Loop is called, as if the
 ...
 */
void taskRestart(Tuint08 uiTaskNumber) defSysWrapperEpilogue;
#define taskRestartOnName(TaskName) taskRestart(preTaskNumberOf(TaskName))
 ...
 */
     ...
     * activated. You can specify anything from 2 to defMaxDelayTime. If you need
...
#define taskFileDirectWrite(Address, Value) portFSWriteByte(Address,Value)
     ...
     */
    #define taskEnableGlobalInterrupts()     do { portTrace(eventAPIcallTaskEnableGlobalInterrupts); portEnableGlobalInterrupts(); } while(false)
     ...
     * Never call Enable/Disable interrupts inside critical sections.
     */
    #define taskDisableGlobalInterrupts()    do { portDisableGlobalInterrupts(); portTrace(eventAPIcallTaskDisableGlobalInterrupts); } while(false)
   ...
   *
   * Call this to unblock context switching. The tick interrupts
   * keep coming, and the tick counter keeps increasing and delayed task
   * wakeup etcetera. Task switching is restored, and high priority tasks
   ...
   *
   * Call this to block context switching. The tick interrupts
   * keep coming, and the tick counter keeps increasing and delayed task
   * wakeup etcetera. Only thing is, there is no task switch, thus by
  ...
  void taskDisableSwitchTask(void);
     ...
     */
      ...
      #define taskEnableTickInterrupts()     do { portTrace(eventAPIcallTaskEnableGlobalInterrupts); portEnableGlobalInterrupts(); } while(false)
    #else
      #define taskEnableTickInterrupts()     do { portTrace(eventAPIcallTaskEnableTickInterrupts); portEnableTickInterrupts(); } while(false)
    #endif
     ...
     */
    #if (cfgIntGlobalOnly == cfgTrue)
      #define taskDisableTickInterrupts()    do { portDisableGlobalInterrupts(); portTrace(eventAPIcallTaskDisableGlobalInterrupts); } while(false)
    #else
      #define taskDisableTickInterrupts()    do { portDisableTickInterrupts(); portTrace(eventAPIcallTaskDisableTickInterrupts); } while(false)
    #endif
    ...
    #define taskExitTickCritical()         taskEnableTickInterrupts()
      ...
      #if (cfgIntSwitchUsesOSstack == cfgTrue)
        /** <!--0404-->
     ...
     * registers utilized inside as well.
     */
...
/** <!--0804-->
 * Call this to find out the number of blocks on a particular combination of events.
 *
