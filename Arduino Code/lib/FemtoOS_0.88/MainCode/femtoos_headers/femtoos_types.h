 ...
 * is kept in two separate bytes, which is more efficient as one 16 bit variable for driect access. However, sometimes
 * it is convineant to have the value as 16 bit word at hand, thus we define a union.
 */
typedef union
{ Tuint16 Full;
  struct
 ...
 * lowest used addresses match) Thus this is a delicate matter.
 */
typedef union
{ TtaskSave pxSave;
 ...
 * values of queus. Furthermore some task statistics are kept. Several members of the
